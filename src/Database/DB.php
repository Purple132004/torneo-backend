...
class DB
{
    ...
    public static function insert(string $query, array $bindings = []): int
    {
        try {
            ...
            // Se stiamo usando PostgreSQL, è più affidabile usare RETURNING id
            $driver = self::getConfig()['driver'] ?? 'mysql';
            $isPg = strtolower($driver) === 'pgsql';

            if ($isPg) {
                // Se la query non include già RETURNING, aggiungilo. Rimuoviamo eventuali punto e virgola finali.
                $q = rtrim($query);
                if (substr($q, -1) === ';') $q = substr($q, 0, -1);
                if (stripos($q, 'returning') === false) {
                    $q .= ' RETURNING id';
                }
            ...
            // Default behaviour per altri driver (MySQL/MariaDB)
            $stmt->execute($bindings);
    ...
    /**
     * Costruisce placeholder dinamici per clausole IN e restituisce i bindings.
     * Esempio: ['clause' => ':p0,:p1', 'bindings' => ['p0' => 1, 'p1' => 2]]
     *
     * @param array $values
     * @param string $prefix
     * @return array
     */
    public static function buildInClause(array $values, string $prefix = 'p'): array
    {
        $placeholders = [];
        $bindings = [];
